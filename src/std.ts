import { TgVldMap, OkTypeMap, Ord, Tuple, Assoc, Dict, Album, Opt, protoChain, ownKeys, ownStringKeys, ownSymbolKeys, hasOwnKey } from './_util.ts'
import { TgVld, union } from './core.ts'
...
export function proto<T extends object>(vld: TgVld<object | null, T>) {
...
export function len<Length extends number>(vld: TgVld<number, Length>) {
...
export function allElems<Tgt, Ok extends Tgt>(vld: TgVld<Tgt, Ok>) {
    return (tgt: Tgt[]): tgt is Ok[] => {
        for (const e of tgt) if (!vld(e)) return false
        return true
    }
}
export function elems<Tgt, Ok extends Tgt, Vlds extends TgVld<Tgt, any>[]>(headVld: TgVld<Tgt, Ok>, ...tailVlds: Vlds): TgVld<Tgt[], [Ok, ...OkTypeMap<Vlds, Tgt>]>
export function elems<Vlds extends TgVld<unknown, any>[]>(...vlds: Vlds): TgVld<unknown[], OkTypeMap<Vlds>>
export function elems(...vlds: TgVld<unknown, any>[]) {
    return (tgt: unknown[]) => {
...
export function allProps<Ok>(vld: TgVld<unknown, Ok>) {
...
export function allPropsStr<Ok>(vld: TgVld<unknown, Ok>) {
...
export function allPropsSym<Ok>(vld: TgVld<unknown, Ok>) {
...
export function hasSchema<OkSchema extends Assoc<unknown>>(vldSchema: TgVldMap<unknown, OkSchema>): TgVld<object, OkSchema & Assoc<unknown>>
export function hasSchema<OkSchema extends Assoc<unknown>, OptKey extends keyof OkSchema = never>(vldSchema: TgVldMap<unknown, OkSchema>, optionalKeys?: OptKey[]): TgVld<object, Opt<OkSchema, OptKey> & Assoc<unknown>>
export function hasSchema(vldSchema: Assoc<TgVld<unknown, any>>, optionalKeys?: (string | symbol)[]) {
...
export function schema<OkSchema extends Assoc<unknown>>(vldSchema: TgVldMap<unknown, OkSchema>): TgVld<object, OkSchema>
export function schema<OkSchema extends Assoc<unknown>, OptKey extends keyof OkSchema = never>(vldSchema: TgVldMap<unknown, OkSchema>, optionalKeys?: OptKey[]): TgVld<object, Opt<OkSchema, OptKey>>
export function schema(vldSchema: Assoc<TgVld<unknown, any>>, optionalKeys?: (string | symbol)[]) {
